...
function DashboardChartsInner({ topProducts, warehouseStats }: DashboardChartsProps) {
  const [pieMetric, setPieMetric] = useState<'qty' | 'value'>('value')

  const maxStock = useMemo(
    () => Math.max(...topProducts.map((p) => p.stock), 1),
    [topProducts]
  )

  const { totalMetric, pieSlices, gradientString } = useMemo(() => {
    const total =
      warehouseStats.reduce(
        (sum, w) => sum + (pieMetric === 'value' ? w.totalValueTRY : w.totalQty),
        0
      ) || 1

    let currentAngle = 0
    const slices = warehouseStats.map((w, index) => {
    ...
    const gradient = slices.map((s) => `${s.color} ${s.startAngle}deg ${s.endAngle}deg`).join(', ')

    return { totalMetric: total, pieSlices: slices, gradientString: gradient }
  }, [warehouseStats, pieMetric])
          ...
          {warehouseStats.length > 0 ? (
            <div className="flex flex-col xl:flex-row items-center gap-6 xl:gap-8">
              ...
              <div className="flex-1 w-full space-y-2 xl:space-y-3">
                {pieSlices.map((slice) => (
                  <div key={slice.name} className="flex items-center justify-between text-xs xl:text-sm group">
                    <div className="flex items-center gap-2">
                      <div 
