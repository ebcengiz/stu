  ...
  return (
    <div className="space-y-4 max-w-[1600px] w-full mx-auto pb-6">
              ...
              <div className="space-y-2 col-span-1 md:col-span-2 lg:col-span-3">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    {customer.category2 && <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold bg-primary-50 text-primary-700 border border-primary-100">{customer.category2}</span>}
                  </div>
                </div>
              </div>
        ...
        <Card className="bg-white shadow-sm border border-gray-200 transform transition-all hover:scale-[1.02]">
          <CardBody className="p-4">
            <div className="flex justify-between items-start">
              ...
              <div className="p-1.5 bg-primary-50 rounded-xl"><ShoppingCart className="h-4 w-4 text-primary-500" /></div>
            </div>
            <div className="mt-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="relative">
          <button onClick={() => { setShowPaymentMenu(!showPaymentMenu); setShowOtherMenu(false); }} className={`w-full h-full flex flex-col items-center justify-center p-3 bg-white rounded-xl border-2 transition-all group shadow-sm ${showPaymentMenu ? 'border-primary-300 bg-primary-50' : 'border-gray-100 hover:border-primary-300 hover:bg-primary-50'}`}>
            ...
            <div className="flex items-center gap-1 font-bold text-[13px] text-gray-900 group-hover:text-primary-700">Tahsilat / Ödeme <ChevronDown className="h-3.5 w-3.5" /></div>
          </button>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 pt-3">
                ...
                <tbody className="bg-white divide-y divide-gray-50">
                  {prevPayments.length > 0 ? prevPayments.slice(0, 20).map(t => (
                    <tr key={t.id} onClick={() => { setSelectedTx(t); setShowTxDetailModal(true); }} className="hover:bg-gray-50 cursor-pointer">
                      ...
                      <td className="px-4 py-2.5 text-sm font-bold text-right text-primary-600">{Number(t.amount).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(t.currency)}</td>
                    </tr>
                  )) : <tr><td colSpan={3} className="px-4 py-6 text-center text-sm text-gray-500">Kayıt bulunamadı.</td></tr>}
                </tbody>
              </table>
        ...
        <Card className="shadow-sm border-gray-200">
          <CardHeader className="bg-gray-50/50 border-b py-3">
            <CardTitle className="text-base font-bold text-gray-900 flex items-center gap-2"><Package className="h-5 w-5 text-primary-500" /> Alışlar / İadeler</CardTitle>
          </CardHeader>
          <CardBody className="p-0">
            <div className="max-h-[400px] overflow-y-auto">
              <table className="min-w-full divide-y divide-gray-100">
          ...
          <Card className="w-full max-w-2xl shadow-2xl rounded-3xl border-0 animate-in zoom-in duration-300">
            <CardHeader className="bg-primary-600 text-white rounded-t-3xl py-6 px-8 flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-xs text-primary-100 mt-1">Müşteriden alınan ödemeyi kaydedin</p>
              </div>
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {ledgerData.map(tx => (
                    <tr key={tx.id} onClick={() => { setSelectedTx(tx); setShowTxDetailModal(true); }} className="hover:bg-gray-50 transition-colors cursor-pointer group">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap"><span className={`px-3 py-1 rounded-lg text-xs font-bold ${tx.isDebt ? 'bg-primary-50 text-primary-700' : 'bg-primary-50 text-primary-700'}`}>{tx.isDebt ? 'Borçlandırma' : 'Alacaklandırma'}</span></td>
                      <td className="px-6 py-4 text-sm text-gray-500 truncate max-w-xs">{tx.description || '-'}</td>
                      <td className={`px-6 py-4 text-sm font-bold text-right whitespace-nowrap ${tx.isDebt ? 'text-gray-900' : 'text-primary-600'}`}>{tx.isDebt ? '+' : '-'}{Number(tx.amount).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(tx.currency)}</td>
      ...
      {showTxDetailModal && selectedTx && (() => {
        const isSale = selectedTx.type === 'sale' || selectedTx.type === 'invoice'
        const isBalanceFix = selectedTx.type === 'balance_fix' || (selectedTx.description?.startsWith('BAKİYE DÜZELTME') ?? false)
        const title = isBalanceFix ? 'Bakiye Düzeltme' : isSale ? 'Satış Detayı' : 'Tahsilat Detayı'
        const TitleIcon = isBalanceFix ? Scale : isSale ? ShoppingCart : CreditCard
        const items = selectedTx.customer_transaction_items ?? []
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[130] p-4 overflow-y-auto animate-in fade-in duration-200">
            <Card className="w-full max-w-3xl max-h-[90vh] flex flex-col shadow-lg animate-in zoom-in duration-200 rounded-xl overflow-hidden border border-gray-200 my-auto">
              ...
              <CardBody className="p-0 bg-white overflow-y-auto flex-1">
                {/* Üst meta bilgileri — yalnızca dolu olanlar render ediliyor, sabit grid yok */}
                {(() => {
                  const metaCells: Array<{ label: string; value: React.ReactNode }> = [
                    ...
                    },
                    {
                      label: 'Tip',
                      value: (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${isSale ? 'bg-primary-50 text-primary-700' : isBalanceFix ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
                          {isSale ? 'Satış (Borç)' : isBalanceFix ? 'Bakiye düzeltme' : 'Tahsilat (Alacak)'}
                        </span>
                      ),
                    },
                  ]
                ...
                <div className="p-6 space-y-5">
                  ...
                  {/* Çek bilgileri */}
                  {selectedTx.type === 'payment' && selectedTx.payment_method === 'cheque' && (selectedTx.cheque_bank || selectedTx.cheque_serial_number || selectedTx.cheque_due_date) && (
                  ...
                  {isSale && (
                    items.length > 0 ? (
                      <div className="rounded-lg border border-gray-200 overflow-hidden">
                        <div className="bg-gray-50 border-b border-gray-200 px-4 py-2.5 flex items-center justify-between">
                          <p className="text-xs font-semibold text-gray-700 uppercase tracking-wide flex items-center gap-1.5">
                            <Package className="h-3.5 w-3.5 text-primary-600" /> Satılan Ürünler
                          </p>
                          ...
                          <table className="min-w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100 bg-white">
                              {items.map((it) => (
                                <tr key={it.id}>
                                  ...
                                  <td className="px-4 py-2.5 text-right text-sm text-gray-700">{Number(it.unit_price).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(selectedTx.currency)}</td>
                                  ...
                                  <td className="px-4 py-2.5 text-right text-sm font-semibold text-gray-900">{Number(it.total_price).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(selectedTx.currency)}</td>
                                </tr>
                              ))}
                            </tbody>
                            <tfoot className="bg-gray-50 border-t border-gray-200">
                              <tr>
                                ...
                                <td className="px-4 py-2 text-right text-sm font-medium text-gray-700">{itemsSubtotal.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(selectedTx.currency)}</td>
                              </tr>
                              {itemsDiscount > 0 && (
                                <tr>
                                  <td colSpan={5} className="px-4 py-2 text-right text-[11px] font-semibold text-gray-500 uppercase tracking-wide">İskonto</td>
                                  <td className="px-4 py-2 text-right text-sm font-medium text-gray-700">-{itemsDiscount.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(selectedTx.currency)}</td>
                                </tr>
                              )}
                              {itemsVat > 0 && (
                                <tr>
                                  <td colSpan={5} className="px-4 py-2 text-right text-[11px] font-semibold text-gray-500 uppercase tracking-wide">KDV</td>
                                  <td className="px-4 py-2 text-right text-sm font-medium text-gray-700">{itemsVat.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(selectedTx.currency)}</td>
                                </tr>
                              )}
                            </tfoot>
                          </table>
                      ...
                      <div className="rounded-lg border border-dashed border-gray-200 bg-gray-50 px-4 py-6 text-center">
                        ...
                        <p className="text-[11px] text-gray-400 mt-0.5">Kalem detayı olmadan tutar üzerinden kaydedilmiş bir satış / borçlandırma.</p>
                      </div>
                    )
                  )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-gray-900 tracking-tight">
                          {Number(selectedTx.amount).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {getCurrencySymbol(selectedTx.currency)}
                        </p>
                      </div>
                    </div>
                  </div>
