  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-full space-y-6 overflow-x-hidden pb-8">
            ...
            <div className="min-w-0 space-y-2 text-sm leading-relaxed text-gray-500">
              <p>
                ...
                <button
                  type="button"
                  onClick={openNew}
                  className="inline-flex items-center gap-1 rounded-full bg-primary-600 px-2.5 py-1 text-xs font-semibold text-white shadow-sm transition hover:bg-primary-700"
                >
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <button
              type="button"
              onClick={openNew}
              className="inline-flex items-center gap-2 rounded-xl border border-primary-200 bg-primary-50/90 px-4 py-2 text-sm font-semibold text-primary-900 shadow-sm transition hover:border-primary-300 hover:bg-primary-100"
            >
              <Plus className="h-4 w-4" />
              Yeni not ekle
            </button>
            ...
            <ul className="space-y-3 border-l-2 border-gray-200">
              {notes.map((n) => (
                <li key={n.id} className="relative -ml-px pl-4 sm:pl-5">
                  <span
                    className="absolute -left-[9px] top-2 flex h-3.5 w-3.5 items-center justify-center rounded-full border border-white bg-white shadow ring-1 ring-primary-200"
                    aria-hidden
                  >
                    <span className="h-1.5 w-1.5 rounded-full bg-primary-600" />
                  </span>
                  <div className="rounded-2xl border border-gray-200/90 bg-white p-3 shadow-sm ring-1 ring-primary-900/5 sm:p-4">
      ...
      {modalOpen && (
        ...
        >
          <div
            className="flex max-h-[min(90vh,560px)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-gray-200/90 bg-white shadow-2xl ring-1 ring-primary-900/10"
          ...
          >
            ...
            <form onSubmit={submitNote} className="flex flex-1 flex-col overflow-y-auto bg-white p-5">
              <label className="mb-1.5 flex items-center gap-2 text-sm font-medium text-gray-600">
                <StickyNote className="h-4 w-4 text-primary-700" />
                Notlar
              </label>
              <textarea
                value={draft}
              ...
              <div className="mt-4 flex justify-end border-t border-gray-100 pt-4">
                <button
                  type="submit"
                  disabled={saving}
                  className="inline-flex items-center gap-2 rounded-xl bg-primary-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-700 disabled:opacity-50"
                >
