...
type ProductBrief = { id: string; name: string }
...
export default function UrunDokumanlarPage() {
  ...
  const [editDoc, setEditDoc] = useState<ProductDoc | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const [rProduct, rDocs] = await Promise.all([
        fetch(`/api/products/${id}`),
        fetch(`/api/products/${id}/documents`),
      ])
      const jProduct = await rProduct.json().catch(() => ({}))
      const jDocs = await rDocs.json().catch(() => ({}))
      if (!rProduct.ok) throw new Error(jProduct.error || 'Ürün yüklenemedi')
      if (!rDocs.ok) throw new Error(jDocs.error || 'Belgeler yüklenemedi')
      setProduct(
        jProduct?.id
          ? { id: String(jProduct.id), name: String(jProduct.name ?? '') }
          : null
      )
  ...
  const submitUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/products/${id}/documents`, { method: 'POST', body: fd })
  ...
  const submitEdit = async (e: React.FormEvent) => {
    ...
    try {
      if (replaceFile) {
        ...
        const res = await fetch(`/api/products/${id}/documents/${editDoc.id}`, { method: 'PATCH', body: fd })
        const data = await res.json().catch(() => ({}))
        if (!res.ok) throw new Error(data.error || 'Güncellenemedi')
      } else {
        const res = await fetch(`/api/products/${id}/documents/${editDoc.id}`, {
          method: 'PATCH',
  ...
  const confirmDelete = (d: ProductDoc) => {
    toast.custom(
      (t) => (
  ...
  const performDelete = async (docId: string) => {
    try {
      const res = await fetch(`/api/products/${id}/documents/${docId}`, { method: 'DELETE' })
      const data = await res.json().catch(() => ({}))
  ...
  const productTitle = (product?.name?.trim() || 'Ürün') as string
    ...
    <div className="mx-auto w-full min-w-0 max-w-full space-y-6 overflow-x-hidden pb-8">
      <div className="overflow-hidden rounded-2xl border border-gray-200/90 bg-white shadow-sm ring-1 ring-gray-900/5">
        ...
        <div className="px-4 py-4 sm:px-6 sm:py-5">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">Ürün belgeleri</p>
        ...
        <div className="rounded-2xl border border-gray-200/90 bg-white p-5 shadow-sm ring-1 ring-gray-900/5 sm:p-6">
          <div className="flex gap-4">
            ...
            <div className="min-w-0 space-y-3 text-sm leading-relaxed text-gray-500">
              <p>
                Bu ürünle ilgili belgeleri (sertifika, teknik şartname, garanti, fiyat listesi…) taratıp veya
      ...
      {uploadModalOpen && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="product-doc-upload-title"
          >
            <div className="flex items-center justify-between bg-gradient-to-r from-primary-600 to-primary-700 px-5 py-4">
              <h2 id="product-doc-upload-title" className="text-base font-semibold text-white">
                Dosya Yükleme
              </h2>
              <button
                type="button"
