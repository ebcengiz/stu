  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (file && file instanceof Blob && file.size > 0) {
        ...
        const newPath = `${tenantId}/loan-documents/${loanId}/${uuidv4()}.${safeExt}`
    ...
    if (body.description !== undefined) {
      updates.description = body.description != null && String(body.description).trim() !== '' ? String(body.description).trim() : null
    }
...
export async function DELETE(
  ...
  try {
    ...
    const { error: delErr } = await supabase.from('loan_documents').delete().eq('id', docId).eq('tenant_id', profile.tenant_id)
