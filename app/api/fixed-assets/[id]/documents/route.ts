...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!fa) return NextResponse.json({ error: 'Demirbaş bulunamadı' }, { status: 404 })
    ...
    const path = `${tenantId}/fixed-asset-documents/${assetId}/${uuidv4()}.${safeExt}`
    ...
    if (uploadError) {
      console.error('fixed asset document upload:', uploadError)
